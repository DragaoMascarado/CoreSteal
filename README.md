...
cardholder name, expiration date, card number
...
service, encrypted token, optional binding key
...
Detects the browser implementation.

Connects to the launcher's local IPC channel.

Reads the App-Bound encrypted key from Chromium's Local State file.

Requests decryption of that key through the browser-specific elevator interface.

Enumerates Chromium profile directories.

Opens the relevant SQLite databases.

Decrypts supported encrypted fields.

Sends structured records to the launcher.
